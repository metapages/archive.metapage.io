...
export class App extends Component {
	...
	load = async () => {
		...
		const moreThanOneParam = hashParams && hashParams['url'] && hashParams['definition'];
		const noParams = Object.keys(hashParams).length == 0;
		if (moreThanOneParam || noParams) {
			this.setState({
				alert: moreThanOneParam ? "Only one hash param allowed (url or definition)" : null,
				metapage: null,
				metapageDefinition: null,
				loadResult: null,
				status: Status.help,
		...
		const paramKey = hashParams['url'] ? 'url' : 'definition'; // if more are supported add here

		// if load is called again, the async portion of this will be cancelled
		const nonce = createNonce();
		let loadResult = { // {key:["url"|"definition"|"metaframe"], alert:<null|string>, nonce:<null|string>}
			alert: null,
			key: paramKey,
		...
		if (key === 'url') {
			...
			if (value) {
				...
				try {
					...
					result.alert = { level: 'error', message: `Failed to JSON.parse #definition: ${err}` };
				}
			}
		}
		return result;
	...
	loadMetapageJson = () => {
		...
		console.log('metapageJsonString', metapageJsonString);
		try {
			// try to parse the JSON string
			JSON.parse(metapageJsonString);
			setMetapageDefinitionInHashParam(JSON.parse(metapageJsonString));
		} catch (err) {
	...
	render() {
		...
		const alertBase64 = this.getAlert('definition');;
		...
		return <div>
			...
			<div class="siimple-card">
				...
				<div class="siimple-card">
					<div class="siimple-card-body">
						<label class="siimple-label">(<code class="siimple-code">#definition=?</code>) containing the base64 encoded metapage definition JSON:</label><br />
	...
	renderMetapage = () => {
		...
		return (
			<div id="app">
				{header}
				<MetapageView definition={metapageDefinition} metapage={metapage} />
			</div>
		);
	}
}
